...
```
You can generate one automatically using `bauplan init <PATH> --name <NAME>`. Note that it scaffolds three files, not one: `bauplan_project.yaml` (the `.yaml` spelling, though both extensions are accepted), plus a `pyproject.toml` and a `models.py` of boilerplate you should get rid of.
