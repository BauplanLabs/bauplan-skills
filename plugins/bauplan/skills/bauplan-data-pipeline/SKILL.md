...
- [ ] Step 12: Offer to add data quality checks → invoke the `bauplan-data-quality-checks` skill with the project path. The skill reads `models.py`, derives checks from how the pipeline uses each table, and generates `expectations.py`.
